...
#'@export
prep_valleys_nlv <- function(Long) {
...
#'@export
metrics_valleys_nlv <- function(master, Long, aspt_col, ffg_col) {
...
#'@export
score_valleys_nlv <- function(Info, Long, aspt_col, ffg_col, scoring){

  metrics <- metrics_valleys_nlv(Info, Long, aspt_col, ffg_col)

  thresh <- matrix(c(4.4, 16.26, 50, 5.78, 1.97, 4.1, 21.57, 53.59,
                             10.56, 2.07), nrow = 5, ncol = 2)
...
#'@export
valleys_nlv <- function(Info, Long, aspt_col, ffg_col, scoring) {
  prep <- prep_valleys_nlv (Long)
  nvl.score <- score_valleys_nlv(Info, prep, aspt_col, ffg_col, scoring)
  return(nvl.score)
}
